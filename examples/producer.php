...
function __publishToExchanges(
    ...
    foreach ($exchangeTypes as $type) {
        ...
        $result = (new Producer($connection))->$method($message, $routingKey, $exchange, []);
...
__publishToExchanges($amqpConnection, $message, 'publishBasic');
__publishToExchanges($amqpConnection, $message, 'publish');
